...
def lambda_handler(event, context):
    ...
    # Check source event time not more than 5 minutes.
    # If it is, consider that the EC2 instance is unhealthy and set it as such.
    ...
    # Start EC2 container instances check
    ...
    for containerListResp in clusterListPages:
        ...
        # Check for empty container instances list
        if not containerListResp['containerInstanceArns']:
            message = "No EC2 container instance available yet, wait a bit."
            logger.info(message)
